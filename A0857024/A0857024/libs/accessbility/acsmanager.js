var AccessibilityManager = (function () {
  ...
  function initAcs(configObj, options, callback) {
    ...
    if (typeof jQuery == "undefined") {
      if (!jQueryScriptOutputted) {
        ...
        document.write(
          "<scr" +
            'ipt type="text/javascript" src="https://code.jquery.com/jquery-3.2.1.min.js" integrity="sha256-hwg4gsxgFZhOsEEamdOYGBf13FyQuiTwlAQgxVSNgt4=" crossorigin="anonymous"></scr' +
            "ipt>"
        );
      }
      setTimeout(function () {
  ...
  function addAcsAttr(elemObj) {
    $.each(elemObj, function (k, v) {
      ...
      if (elemObj[k].docRefObj) {
        currentElem = $(elemObj[k].docRefObj.contentIframe)
          .contents()
          .find(elemObj[k].selector);
      }
        ...
        $.each(currentElem, function (index, el) {
          ...
          if (elemObj[k].dynamicLabel) {
            ...
            $.each(elemObj[k].dynamicLabel, function (idx, elem) {
              ...
              var textElemObj =
                $(el).find(elem)[0] || $(elem)[index] || $(elem)[0];
  ...
  function addAcsEvents() {
    $(document)
      ...
      .on("keydown", function (evt) {
        ...
        if (globalKeyData[key]) {
          ...
          if (
            masterJSON["hotkeys"] &&
            masterJSON["hotkeys"][globalKeyData[key].toLowerCase()]
          ) {
            keyActionObj =
              masterJSON["hotkeys"][globalKeyData[key].toLowerCase()];
          ...
          if (
            tabGroupHotkeys &&
            tabGroupHotkeys[globalKeyData[key].toLowerCase()]
          ) {
          ...
          if (
            dataRef &&
            dataRef["keys"] &&
            dataRef["keys"][globalKeyData[key].toLowerCase()]
          ) {
          ...
          if (keyActionObj) {
            if (keyActionObj.mod) {
              ...
              if (typeof keyActionObj.mod == "string") {
                if (
                  globalKeyData.mods[keyActionObj.mod.toUpperCase()] != true
                ) {
                  flag = false;
                }
            ...
            var actionTarget = keyActionObj.target
              ? $(evt.target).find(keyActionObj.target)[0] ||
                $(keyActionObj.target)[0]
              : $(evt.target)[0];
    ...
    $("body").on("focus", "*", function (evt) {
      ...
      elemRef = $(evt.target).closest("[acsTouch = true]")[0];
      if (
        $(elemRef).attr("checkDisable") &&
        $(elemRef).hasClass($(elemRef).attr("checkDisable"))
      ) {
        return;
      }
  ...
  function acsClickHandler(mainElem, clickTgt) {
    if (
      $(mainElem).attr("checkDisable") &&
      $(mainElem).hasClass($(mainElem).attr("checkDisable"))
    ) {
      return;
    }
    ...
    if (currentTabGroup) {
      ...
      if (obj.docRefObj) {
        domElemRef = $(obj.docRefObj.contentIframe)
          .contents()
          .find(obj.selector)[0];
      }
      if (
        obj &&
        obj.attributes &&
        obj.attributes.disableLibFocus &&
        $(mainElem)[0] == domElemRef
      ) {
    ...
    if (dataRef) {
      ...
      if (dataRef.focusTgt) {
        if (dataRef.focusTgt == "target") {
          ...
          $(dataRef.focusTgt).is(":visible") &&
          $(dataRef.focusTgt).css("display") != "none"
        ) {
    ...
    if (
      currentTabGroup &&
      childTabGroups &&
      childTabGroups[currentChildGroupId]
    ) {
      updateTabIndex(currentTabGroup);
      ...
      cyclicTabbingEnd = $($(cycleGroup[cycleGroup.length - 1]).last())[0];
      if (
        disableFocusTargetAssign == false ||
        searchElementInCycleGroup($(nextFocusTarget)[0]) == false
      ) {
        nextFocusTarget = cyclicTabbingStart;
      }
    ...
    if (
      !cyclicTabbingStart ||
      !$(cyclicTabbingStart).is(":visible") ||
      $(cyclicTabbingStart).css("display") == "none"
    ) {
    ...
    if (
      !cyclicTabbingEnd ||
      !$(cyclicTabbingEnd).is(":visible") ||
      $(cyclicTabbingEnd).css("display") == "none"
    ) {
      var elemRef = getNextActiveElement(
        cycleGroup,
        -1,
        cyclicTabbingEnd || cyclicTabbingStart
      );
      if (elemRef) {
    ...
    setTimeout(function () {
      if (nextFocusTarget != null) {
        ...
        if (
          cyclicTabbing == true &&
          searchElementInCycleGroup($(nextFocusTarget)[0]) == true &&
          activeElementSet.current != $(nextFocusTarget)[0]
        ) {
  ...
  function tabKeyHandler(evt) {
    if (cyclicTabbing == true) {
      if (evt.target == cyclicTabbingEnd) {
        ...
        if (
          searchElementInCycleGroup(cyclicTabbingStart) == true &&
          activeElementSet.current != cyclicTabbingStart
        ) {
  ...
  function shiftTabKeyHandler(evt) {
    if (cyclicTabbing == true) {
      if (evt.target == cyclicTabbingStart) {
        ...
        if (
          searchElementInCycleGroup(cyclicTabbingEnd) == true &&
          activeElementSet.current != cyclicTabbingEnd
        ) {
        ...
        $.each(obj.children, function (k, v) {
          if (v.data && v.data.childTabGroupId) {
            ...
            if (v.ignoreTab) {
              ...
              if (
                childTabGroups[v.data.childTabGroupId].indexOf(v.selector) < 0
              ) {
  ...
  function getNextActiveElement(elemGroup, direction, currentElement) {
    ...
    if (elemPosition >= 0) {
      ...
      for (var i = 0; i < elementsArr.length; ++i) {
        ...
        if (
          pos >= 0 &&
          pos < elementsArr.length &&
          $(elementsArr[pos]).is(":visible") &&
          $(elementsArr[pos]).css("display") != "none"
        ) {
  ...
  function dispatchActionHandler(actionName, actionTarget) {
    if (ActionManager.actions[actionName] != undefined) {
      var tgt =
        $(actionTarget)[0] || $(ActionManager.actions[actionName]["target"])[0];
      if (ActionManager.actions[actionName]["name"]) {
        $(tgt).trigger(ActionManager.actions[actionName]["name"], [
          ActionManager.actions[actionName]["data"],
        ]);
      }
      if (ActionManager.actions[actionName]["callback"]) {
        ActionManager.actions[actionName]["callback"].call(
          tgt,
          ActionManager.actions[actionName]["data"]
        );
      }
    ...
    if (lbl) {
      ...
      if (statesObj !== undefined) {
        if (statesObj.type == "dynamic") {
          str =
            isInitialState == true ? $(statesObj.target).text().trim() : state;
        } else {
          str =
            isInitialState == true
              ? statesObj.statesList["0"][configOptions["language"]]
              : statesObj.statesList[state][configOptions["language"]];
        }
      }
  ...
  var acs = {
    ...
    trapPopupFocus: function () {
      $(document).on("focusin","#lep1_firstElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep1lastbutton").focus();
        },0)
      });
      $(document).on("focusin","#lep1_lastElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep1firstbutton").focus();
        },0)
      });
      $(document).on("focusin","#lep2_firstElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep2lastbutton").focus();
        },0)
      });
      $(document).on("focusin","#lep2_lastElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep2firstbutton").focus();
        },0)
      });
      $(document).on("focusin","#lep3_firstElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep3lastbutton").focus();
        },0)
      });
      $(document).on("focusin","#lep3_lastElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep3firstbutton").focus();
        },0)
      });
      $(document).on("focusin","#lep4_firstElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep4lastbutton").focus();
        },0)
      });
      $(document).on("focusin","#lep4_lastElement", function (evt) {
        setTimeout(function(){
          document.getElementById("lep4firstbutton").focus();
        },0)
      });
    },
    trapConfirmPopupFocus: function () {
      $("#pp_firstElement").focusin(function () {
        var buttons = $(
          ".popupParent .popupContainer button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[buttons.length - 1].focus();
      });
      $("#pp_lastElement").focusin(function () {
        var buttons = $(
          ".popupParent .popupContainer button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[0].focus();
    ...
    trapInstrPopupFocus: function () {
      $("#psi_firstElement").focusin(function () {
        var buttons = $(
          ".playscreenIns button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[buttons.length - 1].focus();
      });
      $("#psi_lastElement").focusin(function () {
        var buttons = $(
          ".playscreenIns button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[0].focus();
    ...
    trapBlankReportFocus: function () {
      $("#bgr_firstElement").focusin(function () {
        var buttons = $(
          ".blankGameReport button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[buttons.length - 1].focus();
      });
      $("#bgr_lastElement").focusin(function () {
        var buttons = $(
          ".blankGameReport button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[0].focus();
    ...
    trapGameReportFocus: function () {
      $("#gr_firstElement").focusin(function () {
        var buttons = $(
          ".gameReport button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[buttons.length - 1].focus();
      });
      $("#gr_lastElement").focusin(function () {
        var buttons = $(
          ".gameReport button[type='button']:not([disabled]):not(.ng-hide)"
        );
        buttons[0].focus();
    ...
    addEscapeEvent: function () {
      $(document)
        ...
        .on("keydown", function (evt) {
          var key = evt.which || evt.keyCode || 0;
          if (key == 27) {
            //APT Close Blank Report
            if ($(".blankGameReport").is(":visible")) {
              angular.element('.blankGameReport .reportCloseIcon').trigger("click");
            }
            if ($(".instructionOverlay").is(":visible"))
            {
              angular.element('.instructionOverlay .playScreenclose.infoCloseGlobal').trigger("click");
            }
            if ($(".instructionOverlay").is(":visible"))
            {
              angular.element('.instructionOverlay .playScreenclose.infoCloseGlobal').trigger("click");
            }
            if ($(".popupParent").is(":visible"))
            {
              angular.element('.popupParent .confirmBtns .cancelBtn').trigger("click");
            }
            if ($(".gameReportContainer").is(":visible"))
            {
              angular.element('.gameReportContainer .reportCloseIcon').trigger("click");
            }
            var $elem = $('.levelMode.challenge.locked')
            if($elem){
              $elem.tooltip("close");
            }
          }
